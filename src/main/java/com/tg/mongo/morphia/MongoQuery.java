...
import com.tg.mongo.bean.ConfirmRewardEvent;
...
//        Query<Setting> qry = datastore.createQuery(Setting.class);
    ...
    public static void queryComment() {
        Datastore datastore = getDatastore();

        Query<ConfirmRewardEvent> qry = datastore.createQuery(ConfirmRewardEvent.class);
//        qry.("{customerId : 11908001}");
//        qry.where("{customerId : 11908001}");

        System.out.println(qry.toString());

        List<ConfirmRewardEvent> list = qry.asList();
        for (ConfirmRewardEvent en : list) {
            System.out.println("ConfirmRewardEvent: " + gson.toJson(en));
        }
    }
}
